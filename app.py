...
def explain_with_metadata(text, metadata_model, scaler):
    ...
    try:
        ...
        raise ValueError("⚠️ The StandardScaler is not fitted. Please fit it during training before saving.")
    ...
    if st.button("Predict"):
        if not user_review.strip():
            ...
            with st.spinner("Making prediction and generating explanations..."):
                ...
                label = "🟢 Genuine Review (OR)" if pred == 0 else "🔴 Fake Review (CG)"
                ...
                try:
                    ...
                    st.write(f"🔴 Fake: {shap_values_meta[0][0]:.2f} &nbsp;&nbsp; 🟢 Genuine: {shap_values_meta[0][1]:.2f}")
                except Exception as e:
    ...
    if st.button("Analyze Review"):
        if not user_review.strip():
            ...
            with st.spinner("Analyzing review and generating explanations..."):
                ...
                label = "🔴 Fake Review (CG)" if bert_pred == 1 else "🟢 Genuine Review (OR)"
...
def main():
    ...
    # Use session state in text area
    user_review = st.text_area("Enter your review:", value=st.session_state.user_review, height=150)
    ...
    if st.button("Analyze Review"):
        ...
        with st.spinner("Analyzing review and generating explanations..."):
            # BERT Prediction
            bert_pred, confidence, bert_probs = predict_bert(user_review, model, tokenizer)
            label = "🔴 Fake Review " if bert_pred == 1 else "🟢 Genuine Review "
            st.subheader(f"Prediction: {label}")
            ...
            st.markdown(f"**💡 Confidence Behind the Answer:** {ensemble_percent}%")
            ...
            if ensemble_percent >= 70:
                ...
                st.warning("⚠️ Low Confidence — Review borderline or unclear")    
